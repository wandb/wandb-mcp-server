...
async def search_wandb_docs(query: str) -> str:
    ...
    with track_tool_execution("search_wandb_docs", "n/a", {"query": query}) as ctx:
        try:
            async with httpx.AsyncClient() as client:
                ...
                if "error" in result:
                    error_msg = result["error"].get("message", "Unknown docs search error")
                    ctx.mark_error(f"docs_error: {error_msg}")
            ...
            ctx.mark_error(f"HTTPStatusError: {e.response.status_code}")
            return json.dumps(
                {
