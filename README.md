...
| `MCP_LOG_PRIVACY_LEVEL` | Telemetry privacy: `off`/`standard` use the authenticated W&B username when available; `strict` uses a SHA-256 pseudonym. Datadog keeps one user field, `usr.id`. Segment uses the authenticated username at `off`/`standard` for analytics continuity, with a key pseudonym fallback, and always uses a pseudonym at `strict`. When analytics is enabled at `off`/`standard`, username enrichment is one best-effort, single-flight lookup per bounded actor client and never changes request authorization; `strict` and disabled analytics skip it. Unset defaults to `off`; empty/invalid values fail startup. Raw tool arguments are never logged. | No |
