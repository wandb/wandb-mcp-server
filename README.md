...
| `MCP_LOG_PRIVACY_LEVEL` | Telemetry privacy: `off`/`standard` include an already-authenticated W&B username when available, otherwise omit user identity (no hash fallback); `strict` uses SHA-256 pseudonyms. Datadog keeps one user field, `usr.id`. Segment always receives a pseudonym, never a raw username. Identity enrichment is cache-only and makes no extra W&B request. Unset defaults to `off`; empty/invalid values fail startup. Raw tool arguments are never logged. | No |
