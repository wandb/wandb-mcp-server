...
| `MCP_LOG_PRIVACY_LEVEL` | Telemetry privacy: `off`/`standard` may include an already-authenticated W&B username in canonical logs and Datadog; `strict` uses SHA-256 pseudonyms. Segment always receives a pseudonym, never a raw username. Identity enrichment is cache-only and makes no extra W&B request. Unset defaults to `off`; empty/invalid values fail startup. Raw tool arguments are never logged. | No |
