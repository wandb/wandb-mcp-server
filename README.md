...
| `MCP_LOG_PRIVACY_LEVEL` | Telemetry privacy: `off`/`standard` use an already-authenticated W&B username when functional work has materialized it; `strict` uses a SHA-256 pseudonym. Datadog keeps one user field, `usr.id`. Segment uses that trusted username at `off`/`standard` for analytics continuity, with a key pseudonym fallback, and always uses a pseudonym at `strict`. Telemetry never creates a W&B client or viewer request. Unset defaults to `off`; empty/invalid values fail startup. Raw tool arguments are never logged. | No |
