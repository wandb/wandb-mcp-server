    ...
    try:
        ...
        if authorization.startswith("Bearer "):
            ...
            if len(bearer_token) < 20 or len(bearer_token) > 100:
                ...
                return JSONResponse(
                    status_code=401,
                    content={"error": "Invalid W&B API key format. Get your key at: https://wandb.ai/authorize"},
...
async def logging_middleware(request, call_next):
    ...
    try:
        ...
        if response.status_code == 404:
            ...
            log.debug("   Available routes: /, /health, /favicon.ico, /favicon.png, /mcp")
            if is_mcp:
                log.debug("   This appears to be an MCP endpoint that wasn't handled")
